...
float4 main(float4 position : SV_Position, float3 color : COLOR0) : SV_Target0
{
	...
	void RecreateDepthTarget( AppState& app )
	{
		...
		depthDesc.debugName = "Cube Depth";
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12CubeWindow";
	...
	{
		MessageBoxA( nullptr, "LightD3D12 Cube failed.", "LightD3D12", MB_ICONERROR | MB_OK );
		return 1;
	}
}
