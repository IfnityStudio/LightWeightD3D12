...
float4 main(float4 position : SV_Position, float2 uv : TEXCOORD0) : SV_Target0
{
	...
	void RecreateTargets( AppState& app )
	{
		...
		colorDesc.debugName = "PostProcess Scene Color";
		...
		depthDesc.debugName = "PostProcess Scene Depth";
		...
		outputDesc.debugName = "PostProcess Compute Output";
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12PostProcessWindow";
		...
		while( app.running )
		{
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "PostProcess::ScenePass", 0xff4cc9f0u );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "PostProcess::ComputePass", 0xff90be6du );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "PostProcess::PresentPass", 0xfff9c74fu );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "PostProcess::ImguiPass", 0xfff9844au );
	...
	{
		MessageBoxA( nullptr, exception.what(), "LightD3D12 PostProcess", MB_ICONERROR | MB_OK );
		return EXIT_FAILURE;
	}
