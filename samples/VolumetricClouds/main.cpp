...
namespace
{
	...
	LoadedModel CreateGroundPlaneModel( RenderDevice& ctx )
	{
		...
		vertexBufferDesc.debugName = "VolumetricClouds Ground Vertex Buffer";
		...
		indexBufferDesc.debugName = "VolumetricClouds Ground Index Buffer";
	...
	void RecreateDepthTarget( AppState& app )
	{
		...
		depthDesc.debugName = "VolumetricClouds Scene Depth";
	...
	ComputePipelineState CreateShapeNoisePipeline( RenderDevice& ctx )
	{
		...
		desc.computeShader = LightHLSLLoader::LoadStage( "shaders/VolumetricCloudsShapeNoiseCS.hlsl", "cs_6_6" );
	...
	{
		ComputePipelineDesc desc{};
		desc.computeShader = LightHLSLLoader::LoadStage( "shaders/VolumetricCloudsDetailNoiseCS.hlsl", "cs_6_6" );
		return ctx.CreateComputePipeline( desc );
	}
	...
	RenderPipelineState CreateGroundPipeline( RenderDevice& ctx, DXGI_FORMAT colorFormat, DXGI_FORMAT depthFormat )
	{
		...
		desc.vertexShader = LightHLSLLoader::LoadStage( "shaders/VolumetricCloudsGroundVS.hlsl", "vs_6_6" );
		desc.fragmentShader = LightHLSLLoader::LoadStage( "shaders/VolumetricCloudsGroundPS.hlsl", "ps_6_6" );
	...
	{
		RenderPipelineDesc desc{};
		desc.vertexShader = LightHLSLLoader::LoadStage( "shaders/VolumetricCloudsVS.hlsl", "vs_6_6" );
		desc.fragmentShader = LightHLSLLoader::LoadStage( "shaders/VolumetricCloudsPS.hlsl", "ps_6_6" );
		desc.color[ 0 ].format = colorFormat;
	...
	void GenerateNoiseTextures( ICommandBuffer& commandBuffer, AppState& app )
	{
		...
		{
			LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "VolumetricClouds::ShapeNoise", 0xff90be6du );
		...
		{
			LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "VolumetricClouds::DetailNoise", 0xfff9c74fu );
			NoisePushConstants pushConstants{};
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12VolumetricCloudsWindow";
		...
		HWND hwnd = CreateWindowExW(
			...
			L"LightD3D12 Volumetric Clouds",
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
		...
		app.noise.shape = CreateNoiseTexture( ctx, "VolumetricClouds Shape Noise", kShapeNoiseSize );
		app.noise.detail = CreateNoiseTexture( ctx, "VolumetricClouds Detail Noise", kDetailNoiseSize );
		...
		while( app.running )
		{
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "VolumetricClouds::GroundPass", 0xff4cc9f0u );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "VolumetricClouds::CloudPass", 0xff90be6du );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "VolumetricClouds::ImguiPass", 0xfff9844au );
