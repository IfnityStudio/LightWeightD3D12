...
namespace
{
	...
	RenderPipelineState CreateScenePipeline( RenderDevice& ctx, DXGI_FORMAT visibleColorFormat, DXGI_FORMAT thermalDataFormat, DXGI_FORMAT depthFormat )
	{
		...
		desc.vertexShader = LightHLSLLoader::LoadStage( "shaders/NightVisionSceneVS.hlsl", "vs_6_6" );
		desc.fragmentShader = LightHLSLLoader::LoadStage( "shaders/NightVisionScenePS.hlsl", "ps_6_6" );
	...
	ComputePipelineState CreateNightVisionPipeline( RenderDevice& ctx )
	{
		...
		desc.computeShader = LightHLSLLoader::LoadStage( "shaders/NightVisionSensorCS.hlsl", "cs_6_6" );
	...
	{
		RenderPipelineDesc desc{};
		desc.vertexShader = LightHLSLLoader::LoadStage( "shaders/NightVisionPresentVS.hlsl", "vs_6_6" );
		desc.fragmentShader = LightHLSLLoader::LoadStage( "shaders/NightVisionPresentPS.hlsl", "ps_6_6" );
	...
	void RecreateTargets( AppState& app )
	{
		...
		colorDesc.debugName = "NightVision Visible Color";
		...
		thermalDesc.debugName = "NightVision Thermal Data";
		...
		depthDesc.debugName = "NightVision Scene Depth";
		...
		outputDesc.debugName = "NightVision Output";
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12NightVisionWindow";
		...
		while( app.running )
		{
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "NightVision::ScenePass", 0xff4cc9f0u );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "NightVision::ComputePass", 0xff90be6du );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "NightVision::PresentPass", 0xfff9c74fu );
			...
			{
				LIGHTD3D12_CMD_SCOPE_NAMED( commandBuffer, "NightVision::ImguiPass", 0xfff9844au );
	...
	{
		MessageBoxA( nullptr, exception.what(), "LightD3D12 Night Vision", MB_ICONERROR | MB_OK );
		return EXIT_FAILURE;
	}
