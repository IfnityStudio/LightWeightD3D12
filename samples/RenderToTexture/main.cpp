...
float4 main(float4 position : SV_Position, float2 uv : TEXCOORD0) : SV_Target0
{
	...
	TextureHandle CreateCubeTexture( RenderDevice& ctx )
	{
		const std::filesystem::path texturePath =
			std::filesystem::path( __FILE__ ).parent_path().parent_path() / "Sprites" / "enemy.png";
		const LoadedImage image = LoadImageRgba8( texturePath );

		TextureDesc textureDesc{};
		textureDesc.debugName = "RenderToTexture Cube Texture";
	...
	void RecreateOffscreenTargets( AppState& app )
	{
		...
		colorDesc.debugName = "RenderToTexture Offscreen Color";
		...
		depthDesc.debugName = "RenderToTexture Offscreen Depth";
	...
	{
		LIGHTD3D12_CMD_SCOPE_NAMED( buffer, "RenderToTexture::RecordScenePass", 0xff4cc9f0u );
	...
	{
		LIGHTD3D12_CMD_SCOPE_NAMED( buffer, "RenderToTexture::RecordPresentPass", 0xff4cc9f0u );
	...
	void RecordImguiPass( ICommandBuffer& buffer, TextureHandle currentTexture, ImguiRenderer* imguiRenderer )
	{
		...
		LIGHTD3D12_CMD_SCOPE_NAMED( buffer, "RenderToTexture::RecordImguiPass", 0xff4cc9f0u );
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12RenderToTextureWindow";
		...
		HWND hwnd = CreateWindowExW(
			...
			L"LightD3D12 Render To Texture",
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
	...
	{
		MessageBoxA( nullptr, "LightD3D12 RenderToTexture failed.", "LightD3D12", MB_ICONERROR | MB_OK );
		return 1;
	}
}
