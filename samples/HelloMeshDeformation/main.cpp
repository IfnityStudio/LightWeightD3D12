...
namespace
{
	enum class DeformationMode : uint32_t
	{
		None = 0,
		Twist = 1,
		Tape = 2,
	...
	struct ScenePushConstants
	{
		...
		XMFLOAT4X4 world = {};
		std::array<float, 4> baseColor = { 0.94f, 0.80f, 0.18f, 1.0f };
		std::array<float, 4> deformation = { 0.0f, 1.0f, 1.0f, 0.0f };
		std::array<float, 4> taper = { 0.0f, 0.65f, 0.35f, 0.0f };
		std::array<float, 4> twist = { 0.0f, 2.4f, 0.0f, 0.0f };
		std::array<float, 4> sphere = { 0.35f, 1.0f, 0.0f, 0.0f };
	};
	...
	struct AppState
	{
		...
		float deformationBlend = 1.0f;
		float taperCenter = 0.0f;
		float taperRange = 0.65f;
		float taperCenterScale = 0.35f;
		float twistCenter = 0.0f;
		float twistRadiansPerUnit = 2.4f;
		float spherifyFactor = 1.0f;
		float spherifyRadius = 1.0f;
		DeformationMode deformationMode = DeformationMode::Twist;
	};
	...
	LoadedModel CreateDuckModel( RenderDevice& ctx )
	{
		...
		vertexBufferDesc.debugName = "HelloMeshDeformation Vertex Buffer";
		...
		indexBufferDesc.debugName = "HelloMeshDeformation Index Buffer";
	...
	RenderPipelineState CreateDuckPipeline( RenderDevice& ctx, DXGI_FORMAT colorFormat, DXGI_FORMAT depthFormat, bool wireframe )
	{
		...
		desc.vertexShader = LightHLSLLoader::LoadStage( "shaders/HelloMeshDeformationVS.hlsl", "vs_6_6" );
		desc.fragmentShader = LightHLSLLoader::LoadStage( wireframe ? "shaders/HelloMeshDeformationWireframePS.hlsl" : "shaders/HelloMeshDeformationSolidPS.hlsl", "ps_6_6" );
	...
	void RecreateDepthTarget( AppState& app )
	{
		...
		depthDesc.debugName = "HelloMeshDeformation Depth";
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12HelloMeshDeformationWindow";
		...
		HWND hwnd = CreateWindowExW(
			...
			L"LightD3D12 Hello Mesh Deformation",
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
		...
		while( app.running )
		{
			...
			ImGui::Begin( "Mesh Deformation HLSL" );
			ImGui::TextWrapped( "Sample based on the rubber duck asset plus the deformation article from ShaderX3. Twist, tape, and tape + twist are implemented in the vertex shader. Spherification is intentionally left as guided TODO work for you." );
			ImGui::Separator();
			...
			int deformationModeIndex = static_cast<int>( app.deformationMode );
			if( ImGui::Combo( "Deformation mode", &deformationModeIndex, kModes, IM_ARRAYSIZE( kModes ) ) )
			{
				app.deformationMode = static_cast<DeformationMode>( deformationModeIndex );
			}

			ImGui::SliderFloat( "Deformation blend", &app.deformationBlend, 0.0f, 1.0f, "%.2f" );
			...
			const bool usesTaper = app.deformationMode == DeformationMode::Tape || app.deformationMode == DeformationMode::TapeTwist;
			const bool usesTwist = app.deformationMode == DeformationMode::Twist || app.deformationMode == DeformationMode::TapeTwist;

			if( usesTaper )
			{
				ImGui::SeparatorText( "Tape settings" );
				ImGui::Checkbox( "Use Hermite taper", &app.useHermiteTaper );
				ImGui::SliderFloat( "Tape center", &app.taperCenter, -1.2f, 1.2f, "%.2f" );
				ImGui::SliderFloat( "Tape range", &app.taperRange, 0.05f, 1.6f, "%.2f" );
				ImGui::SliderFloat( "Tape center scale", &app.taperCenterScale, 0.10f, 2.20f, "%.2f" );
				ImGui::TextWrapped( "The sample tapers along Z by scaling X and Y with a factor derived from the current Z slice." );
			}

			if( usesTwist )
			{
				ImGui::SeparatorText( "Twist settings" );
				ImGui::SliderFloat( "Twist center", &app.twistCenter, -1.2f, 1.2f, "%.2f" );
				ImGui::SliderFloat( "Twist radians per unit", &app.twistRadiansPerUnit, -10.0f, 10.0f, "%.2f" );
				ImGui::TextWrapped( "The twist rotates each vertex around the Z axis with an angle derived from its Z coordinate." );
			}

			if( app.deformationMode == DeformationMode::SpherifyTodo )
			{
				ImGui::SeparatorText( "Spherify settings" );
				ImGui::SliderFloat( "Spherify factor", &app.spherifyFactor, 0.0f, 1.0f, "%.2f" );
				ImGui::SliderFloat( "Sphere radius", &app.spherifyRadius, 0.25f, 2.0f, "%.2f" );
				ImGui::TextWrapped( "The sphere center is the normalized-space origin. Vertices are pushed toward the sphere surface, and normals rotate toward the sphere normal with an axis-angle blend." );
			}
			...
			const XMMATRIX world = modelScale * modelRotationY * modelTranslation;
			...
			solidPush.baseColor = app.deformationMode == DeformationMode::SpherifyTodo
				? std::array<float, 4>{ 0.70f, 0.74f, 0.86f, 1.0f }
				: std::array<float, 4>{ 0.94f, 0.80f, 0.18f, 1.0f };
			solidPush.deformation =
			{
				static_cast<float>( static_cast<uint32_t>( app.deformationMode ) ),
				app.deformationBlend,
				app.useHermiteTaper ? 1.0f : 0.0f,
				0.0f
			};
                        solidPush.taper = {app.taperCenterScale,
                                           std::max(app.taperRange, 0.01f),
                                            app.taperCenter,
                                           0.0f};
			solidPush.twist = { app.twistCenter, app.twistRadiansPerUnit, 0.0f, 0.0f };
			solidPush.sphere = { app.spherifyFactor, app.spherifyRadius, 0.0f, 0.0f };
	...
	{
		MessageBoxA( nullptr, exception.what(), "LightD3D12 Hello Mesh Deformation", MB_OK | MB_ICONERROR );
		return 1;
	}
}
