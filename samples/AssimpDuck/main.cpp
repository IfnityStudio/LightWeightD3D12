...
namespace
{
	...
	LoadedModel CreateDuckModel( RenderDevice& ctx )
	{
		...
		vertexBufferDesc.debugName = "AssimpDuck Vertex Buffer";
		...
		indexBufferDesc.debugName = "AssimpDuck Index Buffer";
	...
	RenderPipelineState CreateDuckPipeline( RenderDevice& ctx, DXGI_FORMAT colorFormat, DXGI_FORMAT depthFormat, bool wireframe )
	{
		...
		desc.vertexShader = LightHLSLLoader::LoadStage( "shaders/AssimpDuckVS.hlsl", "vs_6_6" );
		desc.fragmentShader = LightHLSLLoader::LoadStage( wireframe ? "shaders/AssimpDuckWireframePS.hlsl" : "shaders/AssimpDuckSolidPS.hlsl", "ps_6_6" );
	...
	void RecreateDepthTarget( AppState& app )
	{
		...
		depthDesc.debugName = "AssimpDuck Depth";
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		windowClass.lpszClassName = L"LightD3D12AssimpDuckWindow";
	...
	{
		MessageBoxA( nullptr, exception.what(), "LightD3D12 Assimp Duck", MB_OK | MB_ICONERROR );
		return 1;
	}
}
